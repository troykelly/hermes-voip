...
| Allowlist of file paths (like caller-modes lists) | Dial targets (extensions/SIP URIs) are short allowlist entries, not a PII corpus; a comma-separated inline value (in the gitignored `.env`) matches ADR-0019 §8's stated shape and keeps the config surface minimal. **(Partially reversed by ADR-0099:** a real PSTN-number corpus *is* PII and an env var is shell-visible, so `HERMES_VOIP_OUTBOUND_ALLOW_FILE` was added as an OPTIONAL, union-merged second source — the inline form is retained for the minimal-extensions case.) |
