...
- **Amended by:** ADR-0105 removes the `LIVE_CALL_GUARD_MISSING` current reason and
  adds `VOIP_ORIGIN_NOT_PROACTIVE`; the removed gate branch was a bug that made
  ADR-0074 unreachable, while the added reason code-enforces the inbound VoIP-origin
  fail-safe.
...
allowlisted, a VoIP-owned origin that is never proactive (the inbound fail-safe), or a
non-`place_call` tool.
...
`ORIGIN_UNAVAILABLE`, `ORIGIN_NOT_ALLOWLISTED`, `VOIP_ORIGIN_NOT_PROACTIVE`, and
...
before the platform-scoped helper could decide. ADR-0105 also adds
`VOIP_ORIGIN_NOT_PROACTIVE`, the code-enforced deny for any VoIP-owned platform before
allowlist matching.
...
- The log fires only when the tool is genuinely blocked, so SAFE tools in the
  no-live-call branch stay silent. ADR-0105 changes one security-relevant decision
  intentionally: a VoIP-owned origin is denied before allowlist matching even if an
  operator misconfigures `HERMES_VOIP_PROACTIVE_CALL_FROM` with `voip:*`.
- One more public type (`ProactiveDenyReason` / `ProactiveDecision`) to maintain;
  ...
  from the current type; it remains only in this ADR's history, and adds
  `VOIP_ORIGIN_NOT_PROACTIVE` for the code-enforced inbound fail-safe.
