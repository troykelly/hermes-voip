...
- [x] **[medium] observability** (#474) — Instrument time-to-first-audio latency (INVITE received → first RTP tx)
...
- [x] **[medium] observability** (#471) — Outbound call lifecycle has no structured log events; only `call_loop_started` is emitted for outbound — `outbound_invite_sent`, `outbound_call_connected`, and `outbound_call_failed` are absent, blocking outbound SLO queries. (src/hermes_voip/adapter.py) [TLS leg already covered by #419/1294; #471 closed the WebRTC/WSS-leg gap]
- [x] **[medium] observability** (#423) — `_teardown_call` reads `CallQuality` for the `rtcp_call_quality` event but performs no one-way/no-audio inference and emits no `one_way_audio` or `media_degraded` structured event. (src/hermes_voip/adapter.py, src/hermes_voip/media/engine.py) [STALE — duplicate of item 1295, already shipped in #423 (enhanced #446): `infer_media_anomalies` is live in `_teardown_call`, tested (`test_media_health_inference.py`) + documented (ADR-0102, runbook-0014)]
...
- [x] **[low] ux** (#480) — Sanitize custom polite-decline text before TTS so markdown/URLs/emoji are not spoken raw in deny-mode decline. (`src/hermes_voip/adapter.py`, `src/hermes_voip/media/call_loop.py`) — #480 routes the decline phrase through sanitize_for_speech (raw fallback if it sanitises to empty).
...
<!-- Gap-review batch 2026-07-11 (orchestrate wave 7, wf_c3187e45): 7 items. VERIFIED live: _SIP_URI.fullmatch('sip:1000@example.com:５０６０') -> True; 'maddr' in _ALLOWED_URI_PARAMS. -->
- [x] (#485) **[high] correctness/security** — `refer.py` `_IPV4`/`_AUTHORITY` (lines 104-106) use Unicode-aware `\d` not `[0-9]`, so `_SIP_URI.fullmatch` (the regex `_validate_transfer_target` relies on) accepts fullwidth/Arabic-Indic digit ports+octets (VERIFIED: `fullmatch('sip:1000@example.com:５０６０')` -> True) and folds them verbatim into the wire-emitted `Refer-To:` header — bypassing the agent-supplied transfer-target injection allowlist (the outbound-Request-URI-guard analogue). Same bug class as #479's `_SIPFRAG_STATUS` `\d{3}`->`[0-9]{3}` fix, missed in the SAME file on the security-sensitive URI regexes. Fix: `[0-9]` in both `_IPV4` and `_AUTHORITY`. (`src/hermes_voip/refer.py`)
- [x] (#486, ADR-0112) **[high] security** — `refer.py` `_ALLOWED_URI_PARAMS` allowlists `;maddr`, which per RFC 3261 §19.1.1 OVERRIDES the routed destination while the URI host stays innocuous: a prompt-injected agent calling `transfer_blind(target="sip:1000@trusted-gateway;maddr=<attacker-ip>")` passes `_uri_params_allowed` yet a compliant proxy routes the triggered INVITE to the attacker (covert host-hijack; `transport` similarly can force the leg to cleartext). The module comment/docstring falsely assert the allowlisted params "carry no routing/dialog-seizing capability" (rule 27). Fix: drop `maddr` (reconsider `transport`) from the allowlist, or justify per-deployment AND correct the comment. (`src/hermes_voip/refer.py`)
- [x] (#497, ADR-0113) **[high] robustness** — No ceiling on ACTIVE call duration: `media_timeout_secs` only catches RTP inactivity and the RFC 4028 session timer only catches a dead dialog, so a caller sending continuous RTP holds an admission slot (ADR-0059 `max_calls`, default 8) FOREVER — ~8 such calls permanently `486 Busy Here`-reject every further inbound INVITE while running STT + LLM loops at unbounded operator cost (DoS). No max-call-duration knob exists. Fix: a configurable max-call-duration force-teardown. (`src/hermes_voip/adapter.py`, `src/hermes_voip/config.py`)
- [x] (#501) [done: a per-turn length CAP (`GuardConfig.max_screen_chars`, default 16384) fails an oversized turn CLOSED before normalisation — the PRIMARY fix, since the GIL means an unbounded `normalize()` stalls the loop even off-thread (cross-vendor review flagged offload-alone as insufficient) — PLUS `normalize()` now runs off the loop via `asyncio.to_thread` (defence-in-depth, like the classifier already was). Tests: oversized→RESTRICT+degraded pre-normalise, thread-offload, config validation] **[medium] robustness** — `guard/onnx.py:180` runs `normalize(text)` SYNCHRONOUSLY on the event loop (only the ONNX inference is `asyncio.to_thread`'d), and nothing caps a finalized caller turn's TEXT length (the Endpointer ends a turn on trailing silence only; no per-turn truncation). One very long turn makes normalize()'s multi-pass regex/homoglyph/decode work block the single-threaded loop, stalling RTP receive/jitter/DTMF/TTS for ALL concurrent calls, not just the offender. Fix: cap turn text length before the guard and/or offload `normalize()`. (`src/hermes_voip/guard/onnx.py`, `src/hermes_voip/guard/normalize.py`, `src/hermes_voip/media/call_loop.py`)
- [x] (#495) **[medium] tests/docs** — `EchoCanceller.reset()` (`media/aec.py`) is DEAD code with a false docstring ("Called at call start (engine connect)…"): the engine's per-call reset does `self._aec = None` and `_ensure_aec()` lazily rebuilds a fresh instance (correctly resizing for a cross-call codec-rate change), NEVER calling `.reset()`. Zero callers, zero test coverage (pre-existing — predates #481). Fix: delete `reset()` + its claim, OR wire it in AND make it re-derive the rate-sized state (rule 27). (`src/hermes_voip/media/aec.py`)
- [x] (#498) **[medium] test** — `call_loop.py` `_on_untranscribed_turn`'s "at most one reprompt in flight (no stacked reprompts)" guard (`if existing is not None and not existing.done(): return`, ~line 1937) has ZERO test coverage: the five #1282 tests never fire a second untranscribed turn while the first reprompt task is still speaking, so a regression dropping/inverting the guard (-> two overlapping "didn't catch that" TTS streams into the call) passes CI silently. Fix: add a two-in-flight test. (`tests/test_call_loop.py`)
- [x] (#488) **[low] correctness** — `dialog.py:54` `_CSEQ = re.compile(r"(\d+)\s+(\w+)")` uses Unicode `\d`/`\w`, diverging from the codebase's strict-ASCII posture (#479 named this file as the example to match). Latent — `_cseq()` only runs on our own self-built outbound INVITE today (not attacker-reachable) — but resurfaces the fold-bug class if `_cseq()` is ever reused on a peer-supplied CSeq. Fix: `re.compile(r"([0-9]+)\s+([A-Za-z]+)")`. (`src/hermes_voip/dialog.py`)
- [x] (#506) **[low] security/correctness** — strict-ASCII CSeq **sweep** (attacker-reachable sibling of #488, flagged by cross-vendor review): the CSeq sequence number is parsed via `<value>.split()` then `int(parts[0])` at ~7 sites that handle PEER-supplied CSeqs — `registration.py` (response↔REGISTER match), `transport/connection.py`, `transport/transaction.py`, `transport/ws_connection.py` (transaction/response matching; a test already probes `"² INVITE"`), `call.py` (REFER-progress CSeq match), `adapter.py` (`_CSEQ_PARTS`, `response.header("CSeq").split()`). `int()` folds fullwidth/Arabic-Indic digits, so a peer response carrying a Unicode-digit CSeq can `int()`-match an outstanding ASCII CSeq — a matching differential vs an RFC 3261 §25-strict proxy. Unlike #488's latent outbound-only `_cseq()`, these are the actually-reachable surface. Fix: ONE shared strict-ASCII CSeq-number helper (`[0-9]+`, or reject non-`str.isascii()` before `int()`) applied at every site, each with a red→green test. Do as one coherent sweep, not piecemeal. (`src/hermes_voip/registration.py`, `src/hermes_voip/transport/*.py`, `src/hermes_voip/call.py`, `src/hermes_voip/adapter.py`) [done #506, 2026-07-11: VERIFICATION found the sweep was 6/7 ALREADY complete — `registration.py` (#390), `call.py` `_cseq_number` (#393), `transport/transaction.py`, `transport/connection.py`, `transport/ws_connection.py`, `dialog.py` (#488) all already route the CSeq number through the shared strict-ASCII `_parse_decimal` helper (`isascii()`+`isdecimal()`, fail-closed; the ADR-0081 rollout, tasks #59/#62). The ONE genuinely-unguarded residual was `adapter.py::_cseq_num` (the outbound re-auth W1 stale-407 filter), which still used `.isdigit()` + `int(parts[0])` — a fullwidth/Arabic-Indic digit folds to an ASCII value (matching differential) and a superscript two raises a bare ValueError. Fixed to use `_parse_decimal` (red→green `test_cseq_num_rejects_a_unicode_digit_cseq_number`; cross-vendor codex APPROVE). The CSeq strict-ASCII class is now fully closed.]
- [x] (#502) **[medium] liveness/DoS** — `rtp.py:471` JitterBuffer permanently STALLS after an inbound loss burst >= `max_ahead` packets (agent goes DEAF for the rest of the call). At engine defaults (target_depth=2, max_ahead=256) a gap of >= 256 lost packets (~5.12s at 50pps) — a plausible multi-second mobile/Wi-Fi dropout, or sustained on-path blackholing — freezes the anchor `_next`: on recovery the sequence is > max_ahead past the stale anchor so push() drops every packet (`elif (seq - self._next) % _SEQ_MOD > self._max_ahead: return`, L471) and occupancy never reaches `_depth`, so pop()'s sole loss-advance path (L517) never fires and pop() returns None forever — the engine inbound loop yields NO frames (not even concealment), VAD/ASR/endpointer get permanent silence, and the RTP-inactivity watchdog re-arms on every still-arriving datagram (also default-off, `media_timeout_secs=0.0`) so the call never fails over. Verified via standalone repro (gaps 256/257/300/1000 emit 0 frames, anchor frozen; control gaps 254 AND 255 recover). NOT a cheap injection — needs real >256-packet loss (the S,S+257 injection claim was REFUTED by verify). Fix: a push()-side re-anchor — after a short run of too-far-ahead drops (mirror the ssrc-hysteresis run counter), record the gap as a coalesced Lost and re-anchor `_next`/`_highest` onto the new sequence; also harden pop() to advance to the lowest buffered seq when the anchor is absent and occupancy<depth (fixes the gap==max_ahead/occupancy==1 case). Regression tests for gap==max_ahead and >max_ahead. (`src/hermes_voip/rtp.py`) [REPLENISH wave-15 find+verify confirmed, 2026-07-11] [done #502, 2026-07-11: push()-side re-anchor (`_reanchor_if_stranded`, `_REANCHOR_AFTER=3`, mirroring the ssrc-hysteresis run counter) re-homes `_next` after a confirmed forward run of too-far-ahead packets. GATED on a genuinely-stranded predicate (`_next not in _packets and len(_packets) < _depth` — exactly when pop() returns None and can't advance) so it NEVER evicts still-playable buffered audio, and every non-qualifying arrival (in-window, late, too-far-behind) breaks the run — cross-vendor codex caught both of those in round 1 (APPROVE round 2). The planned pop()-side lowest-buffered advance proved unnecessary: push re-anchor alone recovers gap==max_ahead too (the single edge packet is sacrificed; the stall/DoS is closed). Tests: gap==/>max_ahead (256/257/300/1000) recover, control 254/255 recover via the loss-gate, a far burst does NOT evict a playable buffer, and sub-threshold / non-progressing stranded runs don't re-anchor]
- [x] (#504) **[medium] robustness/DoS-hardening** — `adapter.py` post-answer inbound-INVITE setup (~L3795-3922) runs OUTSIDE the leak-safe try/finally (which only starts ~L3939), so ANY exception between the 200 OK and the finally — guard_state build, CallSession wiring, manager/transport route install, `_start_session_timer`, `extract_call_context`, `_inject_call_context_first_turn` — skips `_teardown_call`/`_release_admission` and permanently leaks the admission slot + bound RTP socket + both in-dialog routes (repeat max_calls -> 486 DoS). The ~L3924-3931 comment already ASSERTS this region is leak-safe ("ANY failure now … must release … never leak them") — rule 27 aspirational. The one demonstrated trigger (a Unicode-digit History-Info/Diversion param raising in `extract_call_context`) is FIXED at root by the call_context `_parse_decimal` fix (this wave); this item is the defense-in-depth backstop for any OTHER post-answer exception. Fix: move the `try:` boundary up to right after the media-setup block (~L3781) so the existing finally covers L3795-3922, with a dialog_id fallback for the not-yet-registered session (`session.dialog_id if session is not None else (call_id, "", "")`), mirroring the outbound post-answer teardown. (`src/hermes_voip/adapter.py`) [REPLENISH wave-18 find+verify confirmed, 2026-07-11] [done #504, 2026-07-11: moved the leak-safe try/finally boundary UP to wrap the whole post-answer region (CallSession wiring, both in-dialog routes, session-timer, `extract_call_context`, `_inject_call_context_first_turn`) plus the CallLoop, so the finally ALWAYS runs `_teardown_call`. `session` starts None with a `dialog_id` fallback `(call_id, "", "")` — `_teardown_call` and `_classify_end_reason` are both None-safe. The polite-decline branch keeps its own AGENT_HANGUP teardown, gated by a `torn_down` flag so the finally never double-tears. Mirrors the outbound pre-session finally; the success and media-setup-failure paths are unchanged. TDD: two `test_adapter_production_safety` tests force `extract_call_context` / `_start_session_timer` to raise post-answer and assert the admission slot + RTP engine + both in-dialog routes are released (red→green). Cross-vendor codex APPROVE.]
- [x] (#491) **[medium] correctness** — `voip_tools.py:1543-1558` `send_dtmf_handler` validates only non-empty + len<=64, NOT the digit alphabet, before dispatch. The default RFC 4733 engine path transmits digits ONE AT A TIME and `digit_to_event` raises lazily on the first non-DTMF char, so a mixed valid-then-invalid string (e.g. an agent formatting `"4111 1111 1111 1111"` — `_str_arg` keeps internal spaces; space ∉ `_DIGITS`) puts the valid PREFIX on the RTP wire, THEN the handler returns `{"error": "invalid DTMF digits"}` under its own "(nothing sent)" contract — so the model re-sends and the far-end IVR gets prefix+full string (corrupted entry/lockout). NOT an exfil channel (DTMF always targets the connected peer) and the surfaced error carries only the single offending char (rule 34 intact); the one broken contract is "(nothing sent)". Fix: pre-validate the WHOLE alphabet in `send_dtmf_handler` (reject if any char ∉ 0-9*#A-D case-insensitive) before `send_dtmf_on_call`, mirroring the length-cap precedent; also pre-validate the whole burst in `CallSession.send_dtmf` (call.py) before delegating, as `send_dtmf_info` already does ("a bad digit never emits a partial burst"). (`src/hermes_voip/voip_tools.py`, `src/hermes_voip/call.py`) [REPLENISH wave-18 find+verify confirmed, 2026-07-11] (shipped #491 — root fix in `CallSession.send_dtmf`, covers all callers)
- [ ] **[low] robustness/DoS-hardening** — WS head-size cap (defense-in-depth follow-up to #492): `ws_connection.py` connects with `max_size=256*1024` and does NOT use `SipMessageFramer`, so a whole 256 KiB WebSocket frame reaches `SipRequest`/`SipResponse.parse` with no separate HEAD cap — unlike the framed TCP/TLS path bounded by `framing._MAX_HEAD=64 KiB`. #492 made unfolding linear so 256 KiB now parses fast (the DoS is closed); this is only defense-in-depth to align the WS path's permitted header size with the framed path. Fix: bound the HEADER portion (up to the blank-line CRLFCRLF) of a WS frame to ~`_MAX_HEAD` before parse — NOT the whole frame (a legitimate large SDP body may exceed 64 KiB, which is why `max_size` is 256 KiB), so split head/body first and cap only the head. (`src/hermes_voip/transport/ws_connection.py`) [wave-22 audit follow-up, 2026-07-11]
- [ ] **[low] auth-hardening** — inconsistent digest challenge-selection (3 sites, one class): registration correctly picks the strongest RFC 8760 challenge via `pick_best_challenge` (`registration.py:630`), but the in-dialog re-INVITE/REFER/INFO path (`call.py:959` `_authorization`) and both outbound-INVITE handlers (`adapter.py:2158` SIP/TLS, `~2979` WebRTC/WSS) read only `response.header(...)` — the FIRST challenge — bypassing `pick_best_challenge`; and `registration.py:629` aborts the whole challenge set if ONE entry is malformed (a robustness gap). VERIFY correctly DOWNGRADED medium→low: hermes-voip has NO plaintext SIP transport (TLS/WSS only), so the MITM header-reorder / forced-MD5-crack vectors are void — only an RFC 8760 §2.4 conformance/internal-consistency gap remains (a server offering SHA-256+MD5 in non-preference order makes these paths use MD5 while REGISTER uses SHA-256). Fix: route the three call/adapter sites through `headers_all(...)` + `pick_best_challenge([...])` (mirror `registration._strongest_challenge`); make challenge-set parsing skip a malformed entry instead of aborting. (`src/hermes_voip/call.py`, `src/hermes_voip/adapter.py`, `src/hermes_voip/registration.py`) [REPLENISH wave-26 find+verify confirmed, 2026-07-11]

## Gap-review 2026-07-11 (Workflow wf_1177f7d3 find+verify — 6 confirmed of 13 candidates, 7 refuted)

- [x] (#510) **[high] correctness/turn-taking** — `media/call_loop.py:1346` the endpointer is driven with the WRONG clock. `_pump`'s `window_index` is a per-FRAME counter (init L1247, `++` L1358, in lockstep with `frames_received`), but `on_event(vad_event)` stamps `_silence_since = event.frame_index` = the VAD WINDOW ordinal (`vad.py` `_next_index`, `endpoint.py:100`); `advance(window_index)` (L1346) then computes `silent_windows_seen = window_index - _silence_since + 1` (`endpoint.py:134`) mixing the two clocks. On the LIVE path an inbound frame is one ptime (20 ms, 160 samples @8k) but the silero VAD window is 256 samples, so the per-frame counter runs ~1.6x faster than the VAD ordinal and the drift grows ~0.375/frame; after ~40 frames (~0.8 s of call) the drift exceeds `silence_windows-1` (=15 for the default 500 ms) and essentially ANY brief caller pause ends the turn within ~1 frame (~20 ms) instead of 500 ms — the agent cuts the caller off mid-sentence, worsening with call age, on the DEFAULT (non-native-EOT sherpa) endpointer path. `endpoint.py.advance()`'s own docstring requires the VAD-window clock; the correct value `latest_vad_window = self._vad.window_index - 1` is ALREADY computed at L1303 and used for the barge-in gate (L1318/L1339) but NOT passed to `advance()`. Fix: pass the VAD-window clock (`latest_vad_window`) to `advance()`, not the per-frame `window_index`; add a call-loop regression with SUB-window frames (the existing 8k tests use window-sized frames so per-frame==per-window and mask it). RE-VERIFY the repro before fixing (two Workflow agents CONFIRMED with a real-class repro: OFFSET at vad_ordinal 21 on frame 35 -> endpointer FIRED frame 36). (`src/hermes_voip/media/call_loop.py`, `src/hermes_voip/media/endpoint.py`) [Workflow wf_1177f7d3 find+verify CONFIRMED, 2026-07-11] [done #510, 2026-07-11: independently RE-VERIFIED with the real VAD+Endpointer (fake model, 160-sample frames) — buggy fires 0ms after OFFSET, fixed 480ms. Root: the local `window_index` fed to `advance()` was a MISNAMED per-frame counter (incremented in lockstep with `frames_received`); `on_event` stamps `_silence_since` from the VAD-window ordinal. Fix = pass `latest_vad_window` (= `self._vad.window_index - 1`, already computed at L1302 for the barge-in gate) to `advance()`, REMOVE the `window_index` duplicate entirely, and relabel the 3 'window' debug/EOT logs to the real vad-window (rule 27). TDD `test_endpointer_driven_by_vad_window_clock_not_per_frame_counter` drives 160-sample SUB-window frames + a scripted VAD (40 voiced then 10 silent windows ~320ms < 500ms) asserting a sub-threshold pause does NOT end the turn; the existing 8k pump tests used 256-sample one-window frames (per-frame==per-window) which masked it. cross-vendor codex APPROVE.]
- [x] (#512) **[medium] liveness/DoS** — `media/call_loop.py:936-937` `_dtmf_buffer` grows without bound under a sub-gap DTMF flood: a non-`#` digit appends (L936) then `_arm_dtmf_flush_timer()` (L937) which cancels + recreates the 2 s (`_DEFAULT_DTMF_INTERDIGIT_MS=2000`) flush task; a connected caller sending RFC 4733 telephone-event digits (2 corroborating end packets per fresh timestamp, `dtmf.py`) faster than 2 s re-arms the timer forever so `_take_dtmf_group` never runs and the only other drain (a `#` terminator) never arrives — the list accumulates one entry per digit for the call lifetime (bounded only by the 4 h max-duration watchdog), while `_mark_caller_active()` (L925) keeps resetting the no-input watchdog so it never reaps the flood. `feed_dtmf_async` has the identical append at L960. Fix: cap `_dtmf_buffer` length on the append path (drop/flush past a sane bound). (`src/hermes_voip/media/call_loop.py`) [Workflow wf_1177f7d3 find+verify CONFIRMED, 2026-07-11] [done #512, 2026-07-11: both `feed_dtmf` / `feed_dtmf_async` append sites route through a new `_buffer_dtmf_digit` helper that DROPS a digit once `_dtmf_buffer` reaches `_MAX_DTMF_BUFFER` (64, matching the outbound `send_dtmf` cap) then still re-arms the timer, so a flood is bounded to 64 buffered digits and the bounded group still flushes on the next `#` / inter-digit gap; legit <=64-digit entries are byte-for-byte unchanged. TDD `test_dtmf_buffer_is_bounded_under_a_sub_gap_flood` floods 500 digits and asserts <= 64 (red at 500). cross-vendor codex APPROVE.]
- [x] (#516) **[medium] robustness/protocol** — `adapter.py:3313-3323` the outbound WebRTC UAC (`_handle_outbound_webrtc_invite`) leaves a confirmed dialog HALF-OPEN on the callee after a post-ACK failure: it sends the ACK at L3124-3127, then the ENTIRE engine setup (construct L3142-3165, `ptime` setter L3169 which raises on non-positive, `engine.connect()` L3170, session wiring L3174-3206) runs BEFORE `session_established=True` (L3286); if any raises, the `finally` (L3313-3323) runs `session.close()` + `remove_call` but sends NO BYE, so the 2xx/ACK dialog dangles on the callee (RFC 3261 §15) until its own session timer reaps it. The SIP sibling `_handle_outbound_invite` handles the identical post-ACK window via an `ack_sent`-gated `_bye_answered_outbound_dialog` (L2663-2664/L2685); the WebRTC path has neither. The comment at L3316-3319 ("added no manager dialog, so there is nothing to tear down") is inaccurate for a post-ACK dialog (rule 27). Fix: add `ack_sent` tracking + BYE via the reusable `_bye_answered_outbound_dialog(transport, dialog)` in the finally when `ack_sent and not session_established`; correct the comment. (`src/hermes_voip/adapter.py`) [Workflow wf_1177f7d3 find+verify CONFIRMED, 2026-07-11] [done #516, 2026-07-12: mirrored the SIP/TLS sibling — pre-bind `dialog: Dialog | None = None` + `ack_sent = False` before the try, set `ack_sent = True` right after the ACK send, and in the finally BYE via the shared `_bye_answered_outbound_dialog(transport, dialog)` when `ack_sent and dialog is not None` (best-effort, rule 37); corrected the rule-27 comment. TDD `test_place_call_over_wss_byes_confirmed_dialog_on_post_ack_failure` injects a post-ACK `engine.connect()` failure and asserts a BYE (red: byes==[]). cross-vendor codex APPROVE. NOTE: needed the hermes-contract-only mypy pass — `adapter.py` + the wss test are excluded from the default `uv run mypy`, so run the explicit hermes-contract mypy file-list locally.]
- [x] (#514) **[medium] robustness/DoS-hardening** — `adapter.py:6120-6122` `_call_info` gains one PERMANENT entry per answered call (keyed by the forgeable SIP Call-ID, holding caller-controlled strings + the rich `InboundCallContext`) and is never popped/reaped: `_teardown_call` only REWRITES it with `ended=True` (L6120-6122); a grep for `_call_info.pop`/`del`/`.clear` in adapter.py returns ZERO; and `disconnect()` (L1477-1484) clears 8 sibling per-call maps but OMITS `_call_info`. An attacker placing N sequential inbound calls (distinct Call-IDs, one at a time so the concurrency cap never trips) grows it without bound over the long-lived adapter's process lifetime (slow memory DoS) — the repo already bounds the analogous engine `_reception` map at 31 via LRU (CWE-400). The entry is set at L3911 BEFORE the decline check, so answer-then-decline callers accumulate too. `get_chat_info`'s docstring (L1728) claims a Call-ID "has been garbage-collected" — cleanup no code performs (rule 27). Fix: reap `_call_info[call_id]` after a grace period post-teardown (retention is only for `send()` late-reply suppression, L1618-1625) OR bound it LRU like `_reception`; add it to `disconnect()`'s clear set. (`src/hermes_voip/adapter.py`) [Workflow wf_1177f7d3 find+verify CONFIRMED, 2026-07-11] [done #514, 2026-07-11: `_teardown_call` now calls `_retain_ended_call_info(call_id)` — registers the ended Call-ID in a bounded FIFO `_ended_call_order` (OrderedDict) and reaps the oldest ended entry's `_call_info` once past `_MAX_ENDED_CALL_INFO` (256); only ended entries are bounded (a live same-Call-ID call still in `_call_sessions` is never evicted) and 256 recent ends cover any late-reply window so `send()` still drops a late reply. `disconnect()` now clears `_call_info` + `_ended_call_order` too. TDD: retention bound (red at cap+100), recent-entry-survives, disconnect-clears. cross-vendor codex APPROVE.]
- [x] (#518) **[low] rule-27** — `sdp.py:1089-1090` the `SessionDescription.parse` comment claims "an unrelated session-level `a=` line is parsed harmlessly and ignored", but a malformed NUMERIC session-level attribute before the first `m=` (`a=ptime:0`/`-1`, `a=maxptime:0`, `a=rtpmap:x`, `a=fmtp:x`) reaches `session_acc.add_attribute` (L1133) OUTSIDE any try/except and raises `SdpError` (`_positive_ms` L863-865 / `int()` L924/L931), aborting the WHOLE parse. Behaviour is fail-closed (offer rejected, no crash/leak) so impact is LOW, but the comment is inaccurate for the numeric tier (DTLS/ICE attrs ARE suppressed via `contextlib.suppress(SdpError)` L948/951/958; ptime/rtpmap/fmtp deliberately are not). Fix: reword the comment (or, if session-level numeric leniency is wanted, wrap `session_acc.add_attribute` like the media-level DTLS/ICE tier). Existing ptime tests place the bad attr AFTER `m=` (media-level), so the session-level branch is untested. (`src/hermes_voip/sdp.py`) [Workflow wf_1177f7d3 find+verify CONFIRMED, 2026-07-11] [done #518, 2026-07-12: reworded the comment to the actual tiered behaviour — unknown session-level attributes are ignored and malformed DTLS/ICE sub-parses are locally suppressed, while malformed numeric attributes/duplicate rtpmap propagate `SdpError` and fail the whole parse closed. cross-vendor codex APPROVE.]
- [x] (#518) **[low] rule-27/dead-code** — `sdp.py:429` `IceCandidate.parse` sets `typ = tokens[7]` with NO membership check against `_ICE_CANDIDATE_TYPES` (L147) and no check that `tokens[6] == "typ"`, so `...5000 typ BOGUS` parses to `typ=="BOGUS"` and `...5000 NOTTYP host` parses to `typ=="host"`; the `typ` field docstring (L384: host/srflx/prflx/relay) is unenforced (no `__post_init__`) and the constants `_ICE_CANDIDATE_TYPES` (L147) + `_ICE_CAND_TYP_IDX` (L140) are DEAD (referenced only at their own definitions). Inert today (peer-parsed `.typ` is never read for logic — only our OWN gathered candidates are re-rendered), so it is a code-quality/dead-constant defect, not exploitable. Fix: validate `tokens[6]=="typ"` and `typ in _ICE_CANDIDATE_TYPES` in `IceCandidate.parse` (wiring the dead constants), raising `SdpError` on violation — the candidate site's `contextlib.suppress(SdpError)` (L958) then drops a bogus candidate rather than keeping it. (`src/hermes_voip/sdp.py`) [Workflow wf_1177f7d3 find+verify CONFIRMED, 2026-07-11] [done #518, 2026-07-12: validated the mandatory `typ` keyword case-insensitively and raised `SdpError` when it is misplaced, so the existing per-candidate suppression drops only that malformed candidate. The original four-type allowlist proposal was corrected during review: RFC 8839 §5.1 uses case-insensitive quoted ABNF and `candidate-types` ends in `/ token`, so known base types are normalised to lowercase while extension types pass through verbatim rather than being falsely rejected. This wires both formerly-dead constants without breaking valid `TYP HOST` or extension candidates. TDD: uppercase keyword/base type, extension acceptance, misplaced-keyword rejection, and whole-offer isolation. First codex review BLOCKED the over-restrictive draft; corrected diff APPROVE.]

## Gap-review 2026-07-12 (Workflow wf_0d7d1084-920 — 9 confirmed novel defects)

Post-#519 REPLENISH review: 12 independent dimensions produced 22 raw candidates; cross-dimension novelty/deduplication retained 6, two-vote adversarial verification retained 5, and the completeness critic added 4/4 findings that each survived two independent votes (34 agents, 0 errors). Full verifier evidence is preserved in the workflow journal; each item below was reproduced or traced on merged main `38e445c` and checked against this backlog.

- [ ] **[high] liveness/protocol** — RFC 4028 non-refresher watchdog never resets after a valid peer refresh. `_run_session_timer`'s `we_refresh=False` branch sleeps once to the teardown deadline and unconditionally hangs up if the call is still live; `CallSession._on_reinvite` answers a peer refresh but never re-arms/signals that watchdog. A legal `Session-Expires: 90;refresher=uac` call is therefore BYE'd at ~60 s even when the peer refreshed at 45 s (the outbound `refresher=uas` case fails analogously). RFC 4028 §10 requires resetting expiry on every received refresh. (`src/hermes_voip/adapter.py:5329`, `src/hermes_voip/call.py:1078`) [Workflow wf_0d7d1084-920: 2/2 CONFIRMED, 2026-07-12]
- [ ] **[high] liveness/resource** — ElevenLabs streaming HTTP has no connect/read timeout: `urlopen(req)` and `response.read()` can block the worker forever on a stalled endpoint. No exception means `FailoverTTS` never engages; cancellation is armed only after `urlopen` returns; teardown's 5 s join then raises and leaves the daemon worker behind while the caller hears silence until another call watchdog intervenes. The sibling Intercom transport already passes an explicit timeout. (`src/hermes_voip/tts/elevenlabs.py:752`) [Workflow wf_0d7d1084-920: 2/2 CONFIRMED, 2026-07-12]
- [ ] **[high] resource/admission** — inbound admission treats an already-present attacker-controlled Call-ID as admitted. Concurrent tagless INVITEs with one Call-ID but distinct transactions (or even retransmission dispatches) each schedule a setup task, while `_admit_inbound`'s `call_id in _admitted_calls` path returns success without consuming another slot. With `max_calls=1`, N such INVITEs can allocate N call stacks/sockets and bypass the cap. Admission must be transaction/dialog-owned or duplicate dispatch must be coalesced; Call-ID alone is not an ownership proof. (`src/hermes_voip/adapter.py:6361`, `src/hermes_voip/transport/connection.py:771`, `src/hermes_voip/transport/ws_connection.py:600`) [Workflow wf_0d7d1084-920 completeness critic: 2/2 CONFIRMED, 2026-07-12]
- [ ] **[medium] SIP interoperability** — semantic header lookup accepts only long names, not RFC 3261 §7.3.3 compact forms `v/f/t/i/m/c`. A compliant compact-header INVITE parses, but `header("Via")`/`From`/`Call-ID` return `None`; `build_response` then raises because no Via is visible, so no 100/180/200 can be sent. Normalize compact names centrally rather than continuing ad-hoc handling for only `l/x/k`. (`src/hermes_voip/message.py:102`) [Workflow wf_0d7d1084-920: 2/2 CONFIRMED, 2026-07-12]
- [ ] **[medium] RTCP robustness** — `ReceptionStats.report_block` passes `int(self._jitter)` without the `_U32` clamp/mask applied to every sibling ReportBlock field. Alternating attacker-controlled RTP timestamps can drive the RFC 3550 jitter EWMA above `2^32-1`; `ReportBlock` then raises every interval, stalling SR/RR for the rest of the call, and the unguarded closing compound builder also drops RTCP BYE. Reproduced with 800 in-sequence alternating timestamps (`jitter=4294967300+`). (`src/hermes_voip/rtcp.py:1057`, `src/hermes_voip/media/engine.py:2755`) [Workflow wf_0d7d1084-920: 2/2 CONFIRMED, 2026-07-12]
- [ ] **[medium] WebRTC cleanup/protocol** — inbound `_setup_webrtc_call` protects DTLS handshake failure but leaves the whole post-success region (`derive_srtcp_sessions`, engine construction/ptime/connect, mux activation, video sender) outside cleanup. Any raise after the 200/ACK and successful handshake reaches the outer admission-only backstop: ICE sockets, guard routes and dialog survive, with no BYE. Mirror `_abort_answered_call`/the SIP-DTLS sibling around this region. (`src/hermes_voip/adapter.py:4473`) [Workflow wf_0d7d1084-920: 2/2 CONFIRMED, 2026-07-12]
- [ ] **[medium] SRTP correctness** — mid-call SDES rekey installs the newly generated outbound SRTP session before validating/constructing inbound. A parsed peer key with an unsupported but syntactically retained lifetime/MKI makes inbound construction raise after outbound has swapped; no 200 advertises the new outbound key, so subsequent agent audio is permanently encrypted under an unknown key. Construct both sessions first, then atomically install both. (`src/hermes_voip/media/engine.py:3598`, `src/hermes_voip/call.py:1200`) [Workflow wf_0d7d1084-920 completeness critic: 2/2 CONFIRMED, 2026-07-12]
- [ ] **[medium] SIP replay/protocol** — inbound dialogs do not retain or validate remote CSeq. Manager routing checks only Call-ID/tags and `CallSession.handle_request` dispatches by method, so an initial `CSeq: 9 INVITE` followed by stale `CSeq: 1 BYE` is accepted with 200 and ends media; replayed INFO/REFER can repeat side effects. RFC 3261 §12.2.2 requires rejecting lower/out-of-order remote sequence numbers. (`src/hermes_voip/dialog.py:72`, `src/hermes_voip/call.py:1025`) [Workflow wf_0d7d1084-920 completeness critic: 2/2 CONFIRMED, 2026-07-12]
- [ ] **[medium] SDP/re-INVITE correctness** — an inbound media-update re-INVITE is never codec-negotiated. A PCMU call re-offered as PCMA-only is classified as `MediaUpdate`, but `_answer_reinvite` returns the original PCMU-only local media and the engine stays PCMU, producing an RFC 3264-invalid 200 and dead/garbled media. Intersect the new offer and update the engine only after a valid negotiation. (`src/hermes_voip/incall.py:288`, `src/hermes_voip/call.py:1173`) [Workflow wf_0d7d1084-920 completeness critic: 2/2 CONFIRMED, 2026-07-12]
