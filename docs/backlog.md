...
- [ ] **[medium] packaging** — Add missing PEP 621 metadata fields to `pyproject.toml`. The `pyproject.toml` is missing standard PEP 621 metadata fields: `license` (SPDX identifier), `authors`/`maintainers`, `keywords`, `classifiers` (trove identifiers: project status, intended audience, environment), and `urls` (homepage, documentation, repository). These fields are expected by package managers and help with discoverability, compliance tooling, and GitHub's license recognition. The package is PUBLIC (CLAUDE.md invariant) and should carry a clearly visible license declaration (`pyproject.toml:1-9`).
- [ ] **[low] packaging** — Add a top-level `LICENSE` file to the repo. The repository lacks a top-level `LICENSE` file. While `pyproject.toml` can declare a license via SPDX identifier, a committed `LICENSE` file is the canonical location that package managers, license-scanning tools, GitHub's automatic license detection, and automated compliance tooling expect to find. The package is PUBLIC (CLAUDE.md invariant) and needs a clearly visible license statement at the repo root.
