...
materializes the all-extras export to `optional-runtime-export.txt` and then
extracts only the `# via hermes-voip` package names from that file. This avoids
the shell pitfall where `python - <<'PY'` consumes stdin for the heredoc script
itself and would otherwise discard the exported requirements stream. That keeps
the scope aligned with the backlog item: all declared optional runtime extras are
licence-gated, while transitive host/runtime dependencies are still covered by
`pip-audit` rather than a fragile string allowlist. The local project
...
uv export --frozen --all-extras --no-dev --no-emit-project --no-hashes \
  > optional-runtime-export.txt
uv run python - optional-runtime-export.txt <<'PY'
import pathlib
import sys
...
for raw_line in export_path.read_text().splitlines():
...
for package in sorted(set(selected)):
    print(package)
PY
rm -f optional-runtime-export.txt
