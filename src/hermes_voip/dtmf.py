...
ADR-0034 adds the **in-band** backend — the last resort for a gateway that
...
# ---------------------------------------------------------------------------
# In-band DTMF (ADR-0010 last resort / ADR-0034): Goertzel detect + tone gen
# ---------------------------------------------------------------------------
...
# Detection thresholds (tuned on 8 kHz clean G.711; ADR-0034 re-measures live).
...
def inband_tone_pcm(
    ...
    *,
    ...
    """Synthesise one key-press as in-band dual-tone PCM16-LE mono (ADR-0034).
...
def _goertzel_power(samples: list[float], freq: int, sample_rate: int) -> float:
    """Goertzel-filter power of ``samples`` at ``freq`` Hz (ADR-0034).
