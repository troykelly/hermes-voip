...
def test_tls_port_alias_wins_over_canonical_port_on_tls() -> None:
    ...
    Corrected from the original "canonical always wins" assertion: a live UCM-class
    gateway provisioner exports both PORT=5060 (cleartext) and TLS_PORT=5061, and the
    tls handshake must target the TLS port. HERMES_SIP_TLS_PORT is a TLS-only alias, so
...
#
# A real UCM-class gateway provisioner exports BOTH the plain/UDP SIP port
