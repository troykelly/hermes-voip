...
:class:`MediaConfig` but consumed NOWHERE (rule-27 drift). ADR-0034 ships the last two
...
* ``sip_info`` and ``inband`` are now IMPLEMENTED (ADR-0034) — they load and resolve to
...
def test_sip_info_mode_loads() -> None:
    """sip_info is implemented (ADR-0034) — it now loads."""
...
def test_inband_mode_loads() -> None:
    """In-band is implemented (ADR-0034) — it now loads."""
...
def test_resolve_auto_without_pt_inband_enabled_g711_is_inband() -> None:
    """Auto + NO PT + in-band ENABLED on a G.711 call => in-band receive (ADR-0034).
