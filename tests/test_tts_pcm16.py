...
that out-of-range inputs saturate rather than overflow/wrap. The conversion
resolves numpy lazily and accepts any array-like, so the tests pass plain float
lists and need only the ``ml`` extra present at runtime (``importorskip``) — no
static numpy import (so they stay clean under the no-ml base mypy gate, the
``test_tts_sherpa_kokoro_real`` idiom).
"""
...
# Imported after the importorskip: importing the module is safe without numpy
# (the conversion resolves it lazily), but the conversion itself needs numpy.
...
def test_out_of_range_saturates_without_overflow() -> None:
    ...
    value near zero; full-scale conversion must saturate to the range endpoints.
    """
    pcm16 = pcm16_from_float32([2.0, -2.0, 1.5, -1.5])
...
def test_output_is_little_endian_int16_width() -> None:
    ...
    pcm16 = pcm16_from_float32([-1.0, 0.0, 1.0])
