"""Version-pin test for the v0.3.1 release.

These tests assert that the package has been bumped to 0.3.1 and that the
CHANGELOG [Unreleased] section has been replaced with a proper [0.3.1] section
...
def test_pyproject_version_is_031() -> None:
    """pyproject.toml [project].version must be 0.3.1 after the release bump."""
...
def test_source_plugin_yaml_version_is_031() -> None:
    """src/hermes_voip/plugin.yaml version must be 0.3.1 after the release bump."""
...
def test_packaging_plugin_yaml_version_is_031() -> None:
    """packaging/.../plugin.yaml version must be 0.3.1 after the release bump."""
    import yaml  # noqa: PLC0415
...
def test_changelog_has_031_section() -> None:
    """CHANGELOG.md must contain a ## [0.3.1] release section."""
    changelog = _CHANGELOG.read_text(encoding="utf-8")
    assert f"## [{_EXPECTED_VERSION}]" in changelog, (
        f"CHANGELOG.md has no ## [{_EXPECTED_VERSION}] section. "
        "Move [Unreleased] entries into a new [0.3.1] section with today's date."
    )


def test_changelog_031_section_is_not_empty() -> None:
    """The ## [0.3.1] section must contain at least one entry."""
    changelog = _CHANGELOG.read_text(encoding="utf-8")
...
def test_changelog_unreleased_link_points_to_031() -> None:
    """[Unreleased] compare link at the bottom must point to v0.3.1...HEAD."""
    changelog = _CHANGELOG.read_text(encoding="utf-8")
    ...
    assert f"v{_EXPECTED_VERSION}...HEAD" in changelog, (
        ...
        "to v0.3.1...HEAD after the 0.3.1 release. Update the link section."
    )


def test_changelog_has_031_compare_link() -> None:
    """CHANGELOG.md must have a [0.3.1] compare link at the bottom."""
    changelog = _CHANGELOG.read_text(encoding="utf-8")
    # E.g.: [0.3.1]: https://github.com/.../compare/v0.3.0...v0.3.1
    assert f"[{_EXPECTED_VERSION}]:" in changelog, (
        "CHANGELOG.md has no [0.3.1]: link at the bottom. "
        "Add a compare link from v0.3.0...v0.3.1."
    )
